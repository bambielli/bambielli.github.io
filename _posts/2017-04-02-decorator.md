...
When sub-classing, behaviors inherited from the parent are statically set at compile time. This leads to valuable compile time checks and *can* reduce the frequency of runtime errors, but introduces unnecessary rigidity to your class hierarchy. 
...
This hits at a core principle of good class design: **classes should be open for extension, but closed for modification.** This is also known as the "Open/Closed Principle".
...
<figure>
 ...
 - `Decorators` can be used to add both new behaviors (like DecoratorA), and even new state (like DecoratorB) to the decorated objects.
